...
void bucket_sort(struct pair *pairs, struct pair *ordered_pairs, unsigned int size);
...
struct srm* srm_new(double Q, unsigned int width, unsigned int height, unsigned int channels, unsigned int borders) {
  ...
  srm->pairs         = malloc(srm->n_pairs * sizeof(struct pair));
  srm->ordered_pairs = malloc(srm->n_pairs * sizeof(struct pair));
